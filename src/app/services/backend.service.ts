...
export class BackendService {
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      console.error(
        `Server-side hata: ${error.status}, hata mesajı: ${error.message}`
      );
  ...
  submitForm(formData: any, options: any): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/api/submit-form`, formData, options)
      .pipe(catchError(this.handleError.bind(this)));
  }

  generateFormId(): Observable<any> {
    return this.http
      .post(`${this.apiUrl}/api/generate-id`, {})
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  addDataBlob(endpoint: string, data: any): Observable<Blob> {
    return this.http
      .post(`${this.apiUrl}${endpoint}`, data, { responseType: 'blob' })
      .pipe(catchError(this.handleError.bind(this)));
  }

  addDataJson(endpoint: string, data: any): Observable<any> {
    return this.http
      .post(`${this.apiUrl}${endpoint}`, data, { responseType: 'json' })
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  updateData(endpoint: string, id: any, data: any): Observable<void> {
    return this.http
      .put<void>(`${this.apiUrl}${endpoint}/${id}`, data)
      .pipe(catchError(this.handleError.bind(this)));
  }
}
