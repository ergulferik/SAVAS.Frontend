...
export class AdminGenelArizaComponent {
  ...
  getData() {
    this.backend.getDataSource('general-failure', this.selectedMonthIndex, this.selectedYear)
      .subscribe({
        next: (res) => {
          this.dataSource = res;
        },
  ...
  onInserting(event: any) {
    this.backend.addDataBlob('general-failure', event.data)
      .subscribe({
        next: () => {
          this.DcToastService.create({
            allowTimeBar: true,
  ...
  onRemove(event: any) {
    this.backend.removeData('general-failure', event.key)
      .subscribe({
        next: () => {
          this.DcToastService.create({
            allowTimeBar: true,
  ...
  onUpdating(event: any) {
    this.backend.updateData('general-failure', event.key, event.newData)
      .subscribe({
        next: () => {
          this.DcToastService.create({
            allowTimeBar: true,
